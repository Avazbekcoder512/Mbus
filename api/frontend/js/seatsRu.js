...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (userId) {
        try {
            const userRes = await fetch(`${api_url}/profile/${userId}?lang=ru`);
            if (userRes.ok) {
    ...
    try {
        showPreloader();
        const response = await fetch(`${api_url}/trip/${tripId}?lang=ru`, {
            method: "GET"
        });
        ...
        <span>${cls.charAt(0).toUpperCase() + cls.slice(1)} — ${priceByClass[cls]} сум</span>
      `;
            ...
            row.forEach(value => {
                ...
                if (value === "driver") {
                    ...
                    if (seatObj.status === "busy") {
                        ...
                        let iconHtml = "";
                        if (gender === "female") {
                            iconHtml = `<img src="../assets/images/female.png" alt="Женщина" class="seat-icon" />`;
                        } else if (gender === "male") {
                            iconHtml = `<img src="../assets/images/male.png" alt="Мужской" class="seat-icon" />`;
                        }
    ...
    function showTicketForm() {
        ...
        selectedPrices.forEach((seatData, seatId) => {
            ...
            form.innerHTML = `
                    <div class="form-header">
                        <div>
                            Номер места: ${seatData.seatNumber}
                            <button class="close-form-btn">×</button>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="input-group">
                            <label for="fullName_${seatId}">Полное имя:</label>
                            <input
                            type="text"
                            id="fullName_${seatId}"
                        ...
                        <div class="input-group">
                            <label for="gender_${seatId}">Пол:</label>
                            <select
                            id="gender_${seatId}"
                            name="gender_${seatId}"
                            required
                            ...
                            <option value="male" ${userData.user?.gender === 'male' ? 'selected' : ''}>Мужской</option>
                            <option value="female" ${userData.user?.gender === 'female' ? 'selected' : ''}>Женский</option>
                            </select>
                        </div>
                        <div class="input-group">
                            <label for="passport_${seatId}">Номер паспорта:</label>
                            <input
                            type="text"
                            id="passport_${seatId}"
                        ...
                        <div class="input-group">
                            <label for="phone_${seatId}">Телефон:</label>
                            <input
                            type="text"
                            id="phone_${seatId}"
                            name="phone_${seatId}"
        ...
        document.getElementById("continue-btn").addEventListener("click", async (e) => {
            ...
            try {
                const response = await fetch(`${api_url}/ticket-pending?lang=ru`, {
                    method: "POST",
                ...
                if (response.ok && result.order) {
                    ...
                    if (result.error === "Данные пользователя неполны! Пожалуйста, заполните информацию!") {
                ...
                btnText.textContent = "Продолжать";
            }
        });
    }
    ...
    async function handleVerification() {
        ...
        if (!code || code.length !== 6) {
            showErrorPopup('Введите 6-значный код!');
            return;
        }

        const order = localStorage.getItem('order');
        if (!order) {
            showErrorPopup('Заказ не найден!');
            return;
        }

        try {
            const response = await fetch(`${api_url}/confirm?lang=ru`, {
                method: 'POST',
                headers: {
