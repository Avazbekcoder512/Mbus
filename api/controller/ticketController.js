...
export const routeFind = async (req, res) => {
  try {
    ...
    if (!departure_date) {
      ...
      const allDates = await tripModel.distinct("departure_date", {
        route: routeDoc._id,
      });
    ...
    const trips = await tripModel.find(query).populate("bus").populate({
      path: "route",
...
export const pendingTicket = async (req, res) => {
  try {
    ...
    for (const passenger of passengers) {
      ...
      const tempTicket = await ticketModel.create({
        ticketId: ticketId,
        passenger_Id: userId,
      ...
      const downloadUrl = `${process.env.BASE_URL}/${tempTicket._id}/download`;
      const qrBuffer = await QRCode.toBuffer(downloadUrl, { type: "png" });

      const fileName = `qr/${tempTicket._id}.png`;
      ...
      tempTicket.qrImage = fileUrl;
      await tempTicket.save();

      await seatModel.findByIdAndUpdate(tempTicket.seat, {
        status: "busy",
        passenger_gender: tempTicket.gender,
      });
    ...
    // const Message = `Limon.uz saytidagi telefon raqamingizni tasdiqlash kodi ${verificationCode}`
...
export const confirmOrder = async (req, res) => {
  try {
    ...
    if (decodedOrder.userId && decodedOrder.userId !== userId) {
      return res
        .status(403)
        .send({ error: "Order token foydalanuvchiga tegishli emas!" });
    }
    ...
    if (!Array.isArray(tempTicketIds) || tempTicketIds.length === 0) {
      return res
        .status(400)
        .send({ error: "Order token noto‘g‘ri yoki chiptalar topilmadi!" });
    }
    ...
    if (!tempTickets || tempTickets.length === 0) {
      return res
        .status(404)
        .send({ error: "Ko‘rsatilgan vaqtinchalik chiptalar topilmadi!" });
    }
